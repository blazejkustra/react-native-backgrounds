export const CAMPFIRE_SHADER = /* wgsl */ `
...
@group(0) @binding(0) var<uniform> params: CampfireParams;
...
fn main(@location(0) ndc: vec2<f32>) -> @location(0) vec4<f32> {
  ...
  let sparkSize = params.scalars.y;
  let fireIntensity = params.scalars.z;
  let smokeIntensity = params.scalars.w;
  ...
  // Apply user color as a tint
  fire = fire * params.colors.rgb;
  
  let finalColor = max(fire, sparks) + smoke;
  return vec4<f32>(clamp(finalColor, vec3<f32>(0.0), vec3<f32>(1.0)), params.colors.a);
}
`;
